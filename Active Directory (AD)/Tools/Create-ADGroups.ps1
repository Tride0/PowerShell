Function Create-ADGroups {
    <#
        .DESCRIPTION
            This script/tool is used to dynamically create AD Groups from a csv file.
        .NOTES
    ...
    Param(
        ...
        [String] $ResultsPath = "$PSScriptRoot\Create_ADGroups_Results_$(Get-Date -Format yyyyMMdd_hhmmss).csv",
        
        [Boolean] $FailureLog = $True,
        [String] $FailureLogPath = "$PSScriptRoot\Create_ADGroups_Failures_$(Get-Date -Format yyyyMMdd_hhmmss).csv",
        
        [Boolean] $Log = $True,
        [String] $LogPath = "$PSScriptRoot\Create_ADGroups_Log_$(Get-Date -Format yyyyMMdd_hhmmss).txt",
    ...
    Begin {
        ...
        $NewADGroupAvailableParameters = (Get-Command New-ADGroup).Parameters.GetEnumerator() | 
    ...
    Process {
        ...
        If ($AttributeNames -notcontains 'GroupScope') {
            Write-Error -Message "$CSVPath does not contain a 'GroupScope' column. It's mandatory." -ErrorAction Stop
            & $CSVPath
            Exit
        }
        If ($AttributeNames -notcontains 'GroupScope') {
        ...
        :CSVInfoForEach Foreach ($Entry in $CSVInfo) {
            ...
            :AttributeNamesForEach Foreach ($Attribute in $AttributeNames) {
                ...
                If ($NewADGroupAvailableParameters -Contains $Attribute) {
                    Add-ToLog -Value "Adding $Attribute to NewADGroup cmdlet Parameters with a value of $($Entry.$Attribute)." -Terminal $False
                    $NewADGroupParameters.$Attribute = $Entry.$Attribute
                }
                Else {
                    Add-ToLog -Value "Adding $Attribute with a value of $($Entry.$Attribute) to the otherAttributes cmdlet Parameter." -Terminal $False
                    If ($NewADGroupParameters.Keys -contains 'OtherAttributes') {
                        $NewADGroupParameters.OtherAttributes.$Attribute = [String]$Entry.$Attribute
                    }
                    Else {
                        $NewADGroupParameters.OtherAttributes = @{ $Attribute = [String]$Entry.$Attribute }
                    }
            ...
            If ([Boolean]$NewADGroupParameters.Path -and ![Boolean]$NewADGroupParameters.Server) {
                $DomainRoot = ($NewADGroupParameters.Path.Split(',') -like 'DC=*') -Join ','
                $Domain = ($NewADGroupParameters.Path.Split(',') -like 'DC=*').Replace('DC=', '') -join '.'

                # Verify Domain exists and is reachable
                If (![adsi]::exists("LDAP://$DomainRoot")) {
                    Add-ToFailureLog -Info $NewADGroupParameters -Note "$Domain not found"
                    Add-ToLog -Value "$Domain not found. Skipping."
                    Continue CSVInfoForEach
                }

                # Verify AD OU exists
                If (![adsi]::Exists("LDAP://$($NewADGroupParameters.Path)")) {
                    Add-ToFailureLog -Info $NewADGroupParameters -Note "AD OU in Path column does not exist $($Entry.Path)"
                    Add-ToLog -Value "$($NewADGroupParameters.Path) does not exist. Skipping."
                    Continue CSVInfoForEach
                }
                ...
                $NewADGroupParameters.Server = $Global:DCs.$Domain
            }
            # Get Current Domain if path or server was not specified
            ElseIf (![Boolean]$NewADGroupParameters.Server) {
            ...
            ElseIf ($NewADGroupParameters.Server -like '*.*') {
                $Split = $NewADGroupParameters.Server.Split('.')
            ...
            Add-ToLog -Value "Running commands against $($NewADGroupParameters.Server). ($Domain)"

            # Create AD Group Object
            Try {
                Add-ToLog -Value "Creating $($NewADGroupParameters.Name)."
                New-ADGroup @NewADGroupParameters -ErrorAction Stop
            }
            Catch {
                Add-ToFailureLog -Info $NewADGroupParameters -Note "Failed to create Group. Error: $_"
                Add-ToLog -Value "Failed to create Group. Error: $_"
            ...
            $NewADGroupParameters = Format-Information -Object $NewADGroupParameters -Headers $AttributeNames

            If ($ExportResults) {
                Add-ToLog -Value "Exporting Results to $ResultsPath."
                ([PSCustomObject]$NewADGroupParameters) | 
