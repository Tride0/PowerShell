Function Find-DirectoryObject {
    ...
    Begin {
        Function FormatInfo {
            Param($ReturnAttribute,[Parameter(ValueFromPipeline)]$User)
            Process {
                $Result = [System.Collections.Specialized.OrderedDictionary]@{}
                
                If ($ReturnAttribute -eq '*') { [String[]]$Attributes = $User.Properties.Keys }
                ...
                Foreach ($Attr in ($Attributes.ToLower() | Sort-Object)) {
                    ...
                    ElseIf ('dn','domain' -contains $Attr) { $SearchAttr = 'distinguishedname' }
                    ...
                    $AttrValue = $($User.Properties.$SearchAttr)

                    Switch ($Attr) {
                        {'lastlogondate','lastlogontimestamp', 'lastlogon','pwdlastset', 'badpasswordtime', 'passwordlastset' -contains $Attr} {
                            $Value = [DateTime]::FromFileTime($Attrvalue)
                            If ($Value -eq '12/31/1600 5:00:00 PM') { $value = '' }
                            $Result.Add($Attr, $Value)
                        }
                        {'parent','parentou','ou' -contains $Attr} {
                        ...
                        # HERE
                        {'useraccountcontrol','uac' -contains $Attr} {
                        ...
                        {'usercertificate','cert','usercert' -contains $_} {
        ...
        If ([Boolean]$PageSize) {
            Write-Verbose -Message "Adding Page Size ($PageSize) to Searcher"
        ...
        If ($ReturnAttribute -ne '*') {
            Foreach ($Attr in $ReturnAttribute) {
                If (!$Searcher.PropertiesToLoad.Contains($Attr)) {
                    ...
                    ElseIf ('dn','parentou','parent','ou','domain' -contains $Attr) {
                    ...
                    ElseIf ('uac','Enabled','LockedOut','Disabled','passwordneverexpires','passwordexpired','passwordnotrequired','passwordcantchange','smartcardrequired' -contains $Attr) {  # HERE
    ...
    Process {
        ...
        Else {
            ...
            Write-Verbose "Filter: $($Searcher.Filter)"
        }
